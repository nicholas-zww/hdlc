                ...
                for i, f in enumerate(frames):
                    ...
                    try:
                        if payload[0] == 0xE0:
                            ...
                            print(f"  WARNING: Unhandled command 0x{payload[0]:02X} in frame {req_id}")
                    except struct.error as e:
                        print(f"  ERROR unpacking payload in frame {req_id}: {e}")
                        continue


        newCrc = crc32_file(args.file)
        if g_crcValue == newCrc:
            print("CRC match")
        else:
            print("CRC mismatch")
    finally:
